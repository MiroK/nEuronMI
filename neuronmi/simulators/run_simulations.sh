...
if [ $# == 0 ]; then
    ...
    mesh_folder=$1
    f=`basename "$mesh_folder"`

    if [ ${mesh_folder: -1} == '/' ]; then
    echo "Simulating: "$mesh_folder$f/$f"_wprobe.h5"    
    python simulate_emi.py -mesh "$mesh_folder$f"_wprobe.h5
    echo "Simulating: "$mesh_folder$f/$f"_noprobe.h5"
    python simulate_emi.py -mesh "$mesh_folder$f"_noprobe.h5 -probemesh "$mesh_folder$f"_wprobe.h5
    else
