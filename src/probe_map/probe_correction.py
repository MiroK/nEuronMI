...
ratio_moi18_hyb = np.max(np.abs(1.8*v_ext_bas), axis=1) / np.max(np.abs(v_ext_hybrid), axis=1)
...
ratio_moi18_emi = np.max(np.abs(1.8*v_ext_bas), axis=1) / np.max(np.abs(v_ext_emi_wprobe), axis=1)
