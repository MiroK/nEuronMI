...
//
// Probe:
// The probe extends from the (z)top of the bbox to z = z_shift. It has
// has rectagle crossection in z plane with size dx_probe and dy_probe. 
// The probe centerline is at 
// x = rad_soma + x_shift + dx_probe/2 and y = y_shift. The probe tip
...
DefineConstant[
  ...
  dx_probe = {0.2, Min 0, Name "Probe size in x dir"}
  dy_probe = {0.2, Min 0, Name "Probe size in y dir"}
...
If(with_probe)
  // Probe
  probe = newv;
  Box(probe) = {rad_soma+x_shift, y_shift-dy_probe/2, z_shift,
                dx_probe, dy_probe, Abs(z_shift)+z1+1};
