...
class TestCases(unittest.TestCase):
    def test_to_DG0(self):
        ...
        df.CompiledSubDomain('near(x[0], 0.5)').mark(facet_f, 1)
    ...
    def test_from_DG0(self):
        def main(n):
            ...
            facet_f = df.MeshFunction('size_t', mesh, 2, 0)
            chi = df.CompiledSubDomain('near(x[0], 0.5)')
            chi.mark(facet_f, 1)
            ...
            dS_ = df.Measure('dS', domain=mesh, subdomain_data=facet_f)
            # Stange that this is not exact
            error = df.inner(df.avg(fV - f), df.avg(fV - f))*dS_(1)
